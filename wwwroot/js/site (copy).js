...
async function fetchName(artistId) {
    ...
    const data = await response.json();
    const artistName = data.item.name || `Artist ${artistId}`;

    artistNameLookup[artistId] = artistName;
    return artistName;
}


// searches a track to get the artist name for that track
async function fetchArtistName(trackId) {
    ...
    const data = await response.json();
    const artistName = data.item.artists[0]?.name || `Artist ${trackId}`;

    trackArtistNameLookup[trackId] = artistName;
    return artistName;
}
...
async function fillQuizData(topTracks, topAlbums, topArtists, trackNames) {
    ...
    if (category === 'artists') {
        const artistNamesPromises = topArtists.map(async artist => ({
            id: artist[0],
            name: await fetchName(artist[0])
        }));
        const artistNames = await Promise.all(artistNamesPromises);
        const artistNamesMap = artistNames.reduce((acc, artist) => {
            acc[artist.id] = artist.name;
            return acc;
        }, {});
        quizData.artists = topArtists.map(artist => artistNamesMap[artist[0]] || artist[0]);
        quizDataCounts.artists = topArtists.map(artist => artist[1]);
    }

    if (category === 'songs') {
        const trackArtistsPromises = topTracks.map(async track => ({
            id: track[0],
            artist: await fetchArtistName(track[0])
        }));
        const trackArtists = await Promise.all(trackArtistsPromises);
        const trackArtistsMap = trackArtists.reduce((acc, track) => {
            acc[track.id] = track.artist;
            return acc;
        }, {});
        quizData.songs = topTracks.map(track => trackNames[track[0]] || track[0]);
        quizDataCounts.songs = topTracks.map(track => ({
            count: track[1],
            artist: trackArtistsMap[track[0]]
        }));
    }
...
function submitGuess() {
    ...
    for (let i = 0; i < answers.length; i++) {
        ...
        if (fuzzyMatch(cleanGuess, cleanAnswer) || fuzzyMatch(guess, answers[i].toLowerCase())) {
            ...
            if (!alreadyDisplayed) {
                if (category === 'songs') {
                    listItems[i].innerText = `${answers[i]} (${answersCounts[i].artist}) - ${answersCounts[i].count.toLocaleString("en-US")} streams`;
                } else if (category === 'albums') {
                    listItems[i].innerText = `${answers[i]} (${answersCounts[i].artist}) - ${answersCounts[i].count.toLocaleString("en-US")} streams`;
                } else {
                    listItems[i].innerText = `${answers[i]} - ${answersCounts[i].toLocaleString("en-US")} streams`;
                }
                found = true;
            }
        }
...
function showMissingAnswers(answers, answersCounts, listItems) {
    for (let i = 0; i < answers.length; i++) {
        if (listItems[i].innerText === '') {
            ...
            listItems[i].innerText = displayText;
...
async function getTopItems(data, key, isArray = false) {
    ...
    if (key === 'trackId') {
        ...
        const trackArtistsPromises = topItems.map(async item => ({
            id: item.id,
            name: trackNames[item.id],
            artist: await fetchArtistName(item.id),
            count: item.count
        }));
        const result = await Promise.all(trackArtistsPromises);
        return result;
    } else if (key === 'albumId') {
        const albumDataPromises = topItems.map(async item => ({
            id: item.id,
            data: await fetchAlbumData(item.id),
            count: item.count
        }));
        const albumData = await Promise.all(albumDataPromises);
        ...
        const artistNamesPromises = topItems.map(async item => ({
            id: item.id,
            name: await fetchName(item.id),
            count: item.count
        }));
        const result = await Promise.all(artistNamesPromises);
        return result;
    }
    return topItems;
}
...
async function calculateAndDisplayStats(data, startTime, endTime) {
    ...
    const top10SongsWithArtists = await Promise.all(top10Songs.map(async song => ({
        ...song,
        artist: await fetchArtistName(song.id)
    })));
    ...
    if (document.getElementById('stats-time-range').innerText == "Your All Time Spotify History") {
        ...
        document.getElementById('top-new-track-value').innerHTML = topNewTracksWithNames.map(track => `<p>${track.name} (${track.artist}) - ${track.count.toLocaleString("en-US")} streams</p>`).join('');
        document.getElementById('top-new-artist-value').innerHTML = topNewArtistsWithNames.map(artist => `<p>${artist.name} - ${artist.count.toLocaleString("en-US")} streams</p>`).join('');
        document.getElementById('top-new-album-value').innerHTML = topNewAlbumsWithNames.map(album => `<p>${album.name} (${album.artist}) - ${album.count.toLocaleString("en-US")} streams</p>`).join('');
    ...
    document.getElementById('top-tracks-value').innerHTML = topTracks.map(item => `<p>${item.name} (${item.artist}) - ${item.count.toLocaleString("en-US")} streams</p>`).join('');
    document.getElementById('top-artists-value').innerHTML = topArtists.map(item => `<p>${item.name} - ${item.count.toLocaleString("en-US")} streams</p>`).join('');
    document.getElementById('top-albums-value').innerHTML = topAlbums.map(item => `<p>${item.name} (${item.artist}) - ${item.count.toLocaleString("en-US")} streams</p>`).join('');
...
document.getElementById('go-back-stats').addEventListener('click', function () {
    document.getElementById('stats-screen').style.display = 'none';
    document.getElementById('stats-options').style.display = 'block'; // Make sure you have an element with id="stats-options"
});
    ...
    for (let rank = 0; rank < 100; rank++) {
        ...
        dateKeys.forEach((dateKey, index) => {
            ...
            if (track) {
                ...
                track.artist = fetchArtistName(track.trackId);
